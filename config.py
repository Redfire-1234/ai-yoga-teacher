...
class Settings(BaseSettings):
    # API Keys
    GROQ_API_KEY: str = os.getenv("GROQ_API_KEY", "")
    HF_TOKEN: Optional[str] = os.getenv("HF_TOKEN", None)  # HuggingFace token for embeddings API
    ...
    # Embedding Model - Using HuggingFace Inference API (no local model loading!)
    # This saves RAM on Render free tier
    EMBEDDING_MODEL: str = "sentence-transformers/all-MiniLM-L6-v2"
    # Note: Model runs on HF servers, not locally!
